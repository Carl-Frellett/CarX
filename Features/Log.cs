...
namespace CarX.API.Features
{
    public static class Log
    {
        ...
        {
            if (canBeSent) Send(message, LogLevel.Debug, System.ConsoleColor.Green);
        }
        ...
        {
            string finalMessage = $"[{Assembly.GetCallingAssembly().GetName().Name}] [{level.ToString().ToUpper()}] {message}";
